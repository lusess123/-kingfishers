    ...
    export class MenuOrgPageVm extends domCore.DomVm {
        ...
        public addMenu(name: string, types: string, rightValue: string) {
            if (types == "Menu") {
                ...
                urlFile.Core.AkPost("/RightCloud/RightConfig/getFid", {}, (data) => {
                    if (data) {
                        var newbtn = { FID: data, ButtonName: name, Org: "", ParentId: this.MenuEditData.FID, ActionType: "Add", IMenuButton: null, RightValue: this.MenuEditData.RightValue };
        ...
        public fun_Save() {
            ...
            urlFile.Core.AkPost("/RightCloud/RightConfig/MenuOrgSubmit", { data: menu }, (data) => {
                if (data) {
                    //刷新  
                    if (data) {
                        //urlFile.Core.AkUrl.Current().openUrl("$RIGHTMAINPAGE$", true);
                        this.emitAppEvent("MainDom/MenuOrgPage", this.UniId, data);
                    }
                }
            });
        }
