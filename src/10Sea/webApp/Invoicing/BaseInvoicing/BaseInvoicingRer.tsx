import WinDefaultRer = require("./../../Default/default/win/WinDefaultRer");
import urlFile = require("./../../../../01core/Url");
import cFile = require("./BaseInvoicingCer");
import utilFile = require("./../../../../01core/Util");

export class BaseInvoicingRer extends WinDefaultRer.WinDefaultRer{
    public getCer(): cFile.BaseInvoicingCer
    {
        return utilFile.Core.Util.Cast<cFile.BaseInvoicingCer>(this.C);
    }

   public $Win = null;
   public SerialNo = null; //流水号
   public VoucherNo = null; //合同编号
   public CreateTime = null;    //制单日期
   public $PageObj = null;
   public Page = null;
   public $TopButtons = null; //头部按钮
   public $BottomButtons = null; //底部按钮
   public $Title = null;
   public Title = null;            //标题
   public MasterTableName = null;  //主表表名
   public TotalAmount = null; //总金额
   public TotalQty = null; //总数量
   public TotalAmountBig = null; //合计(大写)
   ...
   public loadModuleXmlMain(res, pageStyle) {
    ...
    public getUpdateFieldsValue(){
        this.getCer().getUpdateFieldsValue(this.$PageObj);
    };

    public getDetailFieldsValue() {
    ...
    //设置表头样式
    public setTHStyle(col, style, val) {
        this.$PageObj.find("th[acol='" + col + "']").css(style, val);
    };

    //设置单元格样式
    public setTDStyle (col, style, val) {
    ...
    public setInsertDefaultValue () {
        this.getCer().setInsertDefaultValue(this.$PageObj);
    };

    //将对象加上小数点,取小数点后length位
    public setNumberFixed = function (colObj, length) {
...
$.AKjs.Public.addTotalRow = function (formObj, name, html) {//新增表单最后一行
    var _bodyFormObj = formObj.getControlObjBySubmitSign("" + name + "").FormObj;
    _bodyFormObj.initTailRowOpt(html);
}

$.AKjs.Public.addDetailTotalRow = function (formObj, html) {//新增表单最后一行
