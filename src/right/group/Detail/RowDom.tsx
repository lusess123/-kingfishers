...
import groupRightDetailFile = require("./../GroupRightDetailPage");
...
export module Row {
    ...
    export class RowReact extends domCore.DomReact<RowProps, RowStates, RowAction> implements domCore.IReact {
        ...
        public pSender(): React.ReactElement<any> {
            return <div className={"Hg-overflow-auto " + (this.props.Vm.IsItemFormHide ? "  " :" acsHeightInherit")}>
                <div className="Hu-row-title Hu-pointer" onClick={() => { this.fun_itemTitleClick(); } }>{this.props.Vm.Index + 1}
                    ...
                    <div>
                        <h4>权值分配信息：</h4>
                    {this.props.Vm.GroupRightDetail.intoDom() }
                     </div>
                    </div>
                </div>;
        }
    ...
    {
        GroupRightDetail: groupRightDetailFile.GroupRightDetailPage.IGroupRightDetailPageConfig;
    }
    ...
    export class RowVm extends domCore.DomVm {
        ...
        public GroupRightDetail: groupRightDetailFile.GroupRightDetailPage.GroupRightDetailPageVm;

        public constructor(config?:IRowVm)
        {
            super();
            if (config) {
